...
export default function useSocket(options: UseSocketOptions) {
  ...
  const setupEventHandlers = useCallback(() => {
    ...
    // Connection events
    socket.on('connect', () => {
      console.log(`[Socket ${instanceId.current}] Socket conectado ID: ${socket.id}`);
      setConnected(true);
      identifyUser();
  ...
  const joinConversation = useCallback((conversationId: string) => {
    if (!socketRef.current || !connected || !conversationId) {
      console.error(`[Socket ${instanceId.current}] No se puede unir a la conversación: socket no conectado o conversación inválida`);
      return false;
    }
